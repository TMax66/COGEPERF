...
fterep <- tabIZSLER %>% 
  ...
  summarise_at(c("FTED"), sum, na.rm = T)  
    ...
    #         Reparto, "SEDE TERRITORIALE DI FORLÃŒ - RAVENNA" = "SEDE TERRITORIALE DI FORLÌ - RAVENNA"))),
    #   by = c("Dipartimento", "Reparto")) %>% 
    # filter(FTED > 0 & !is.na(FTp))  %>% 
    # mutate(ftepub = FTED*((100-FTp)/100)) 
    #
...
## IF /FTED reparto------  SISTEMARE QUESTO CODICE SEGUENDO IL CODICE SOPRA PER IL RFTE
  # tabIZSLER %>% filter(MESE == 1) %>% 
